...
import { AddIcon, DeleteIcon, SettingIcon, ChartBarIcon } from 'tdesign-icons-react';
    ...
    >
      ...
      <div className="px-3 space-y-1.5">
        <Button 
          ...
          block
          variant={isAnalysisPage ? 'base' : 'outline'}
          theme={isAnalysisPage ? 'primary' : 'default'}
        >
          📊 IM 分析
        </Button>
        <Button 
      ...
      {!isAnalysisPage && (
        <div className="flex-1 overflow-y-auto p-2 space-y-1 mt-2">
          <div className="text-xs px-3 py-1.5" style={{ color: 'var(--td-text-color-placeholder)' }}>
            对话历史
          </div>
          {sessions.map(session => {
            ...
            return (
              <div 
                ...
                style={{
                  backgroundColor: session.id === currentSessionId && !isSettingsPage && !isAnalysisPage
                    ? 'var(--td-brand-color-light)' 
                    : 'transparent',
                  color: session.id === currentSessionId && !isSettingsPage && !isAnalysisPage
                    ? 'var(--td-brand-color)' 
                    : 'var(--td-text-color-secondary)'
                }}
                onClick={() => onSelectSession(session.id)}
                onMouseEnter={(e) => {
                  if (session.id !== currentSessionId || isSettingsPage || isAnalysisPage) {
