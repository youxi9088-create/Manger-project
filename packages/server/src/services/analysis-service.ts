...
export type TopSender = { sender_name: string; message_count: number };
...
export function parseMarkdownReport(markdown: string): any {
  const result: any = {
    summary: "",
    work_priorities: [],
    completed_tasks: [],
...
export function buildAnalysisPrompt(
  date: string,
  chatRecords: Array<{ group_name?: string | null; sender_name?: string; content?: string; timestamp?: string }>,
  stats: AnalysisStats,
  ...
  // ---- 按群名分组聊天记录 ----
  const groupedRecords = new Map<string, typeof chatRecords>();
  ...
  for (const [groupName, records] of sortedGroups) {
    ...
    groupSections.push(
      `=== 【${groupName}】（${records.length} 条消息，${uniqueSenders} 人参与）===\n${lines}`
    );
  }
