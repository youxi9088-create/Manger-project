...
// 创建数据库连接
const db = new Database(dbPath) as BetterSqliteDatabase;
// 启用 WAL 模式以提高性能
...
try {
  const tableInfo = db.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='work_cycles'").get() as { sql: string } | undefined;
...
try {
  const empTableInfo = db.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='employees'").get() as { sql: string } | undefined;
...
try {
  ...
  const hasOldRoles = db.prepare(
    `SELECT COUNT(*) as cnt FROM employees WHERE agent_type IN ('project_delivery','code_dev','qa')`
  ).get() as { cnt: number };
...
try {
  const piColumns = db.prepare("PRAGMA table_info(project_initiations)").all() as { name: string }[];
  const piColNames = piColumns.map(c => c.name);
  const piAdd: Array<{ col: string; def: string }> = [];
  if (!piColNames.includes('deadline')) piAdd.push({ col: 'deadline', def: 'TEXT' });
  if (!piColNames.includes('start_date')) piAdd.push({ col: 'start_date', def: 'TEXT' });
  if (!piColNames.includes('completed_at')) piAdd.push({ col: 'completed_at', def: 'TEXT' });
  if (!piColNames.includes('team_size_required')) piAdd.push({ col: 'team_size_required', def: 'INTEGER DEFAULT 1' });
  if (!piColNames.includes('team_size_current')) piAdd.push({ col: 'team_size_current', def: 'INTEGER DEFAULT 0' });
  if (!piColNames.includes('current_phase')) piAdd.push({ col: 'current_phase', def: "TEXT DEFAULT 'draft'" });
  if (!piColNames.includes('phase_status')) piAdd.push({ col: 'phase_status', def: "TEXT DEFAULT '{\"initiation\":0,\"requirement\":0,\"planning\":0,\"execution\":0,\"delivery\":0}'" });
  if (!piColNames.includes('risk_level')) piAdd.push({ col: 'risk_level', def: "TEXT DEFAULT 'low'" });
  if (!piColNames.includes('risk_reason')) piAdd.push({ col: 'risk_reason', def: 'TEXT' });
  if (!piColNames.includes('risk_alert_at')) piAdd.push({ col: 'risk_alert_at', def: 'TEXT' });
  if (!piColNames.includes('parent_project_id')) piAdd.push({ col: 'parent_project_id', def: 'TEXT' });
  if (!piColNames.includes('external_project_id')) piAdd.push({ col: 'external_project_id', def: 'TEXT' });
  ...
  if (piColNames.includes('current_phase') || piAdd.some(x => x.col === 'current_phase')) {
    try { db.exec('CREATE INDEX IF NOT EXISTS idx_project_initiations_phase ON project_initiations(current_phase)'); } catch {}
  }
  if (piColNames.includes('risk_level') || piAdd.some(x => x.col === 'risk_level')) {
    try { db.exec('CREATE INDEX IF NOT EXISTS idx_project_initiations_risk ON project_initiations(risk_level)'); } catch {}
  }
  if (piColNames.includes('parent_project_id') || piAdd.some(x => x.col === 'parent_project_id')) {
    try { db.exec('CREATE INDEX IF NOT EXISTS idx_project_initiations_parent ON project_initiations(parent_project_id)'); } catch {}
  }
  if (piColNames.includes('external_project_id') || piAdd.some(x => x.col === 'external_project_id')) {
    try { db.exec('CREATE INDEX IF NOT EXISTS idx_project_initiations_ext_id ON project_initiations(external_project_id)'); } catch {}
  }
} catch (e: any) {
  if (!e.message?.includes('duplicate column name')) {
...
try {
  const dtColumns = db.prepare("PRAGMA table_info(dev_tasks)").all() as { name: string }[];
  const dtColNames = dtColumns.map(c => c.name);
  const dtAdd: Array<{ col: string; def: string }> = [];
  if (!dtColNames.includes('start_date')) dtAdd.push({ col: 'start_date', def: 'TEXT' });
  if (!dtColNames.includes('due_date')) dtAdd.push({ col: 'due_date', def: 'TEXT' });
  if (!dtColNames.includes('sprint_week')) dtAdd.push({ col: 'sprint_week', def: 'INTEGER' });
...
try {
  const versionColumns = db.prepare("PRAGMA table_info(versions)").all() as { name: string }[];
  const colNames = versionColumns.map(c => c.name);
  const toAdd: Array<{ col: string; def: string }> = [];
  if (!colNames.includes('goals')) toAdd.push({ col: 'goals', def: "TEXT DEFAULT '[]'" });
  if (!colNames.includes('computed_risk')) toAdd.push({ col: 'computed_risk', def: "TEXT DEFAULT 'unknown'" });
  if (!colNames.includes('risk_reasons')) toAdd.push({ col: 'risk_reasons', def: "TEXT DEFAULT '[]'" });
  if (!colNames.includes('last_check_at')) toAdd.push({ col: 'last_check_at', def: 'TEXT' });
...
try {
  const reqColumns = db.prepare("PRAGMA table_info(requirement_analyses)").all() as { name: string }[];
  const colNames = reqColumns.map(c => c.name);
  const toAdd: Array<{ col: string; def: string }> = [];
  if (!colNames.includes('version_id')) toAdd.push({ col: 'version_id', def: 'TEXT' });
  if (!colNames.includes('from_goal_id')) toAdd.push({ col: 'from_goal_id', def: 'TEXT' });
  for (const { col, def } of toAdd) {
...
try {
  const reqCols = db.prepare("PRAGMA table_info(requirement_analyses)").all() as { name: string }[];
  const verCols = db.prepare("PRAGMA table_info(versions)").all() as { name: string }[];
...
export interface DbAnalysisReport {
  ...
  report_date: string;
  summary: string;
  work_priorities: string | null; // JSON string
...
export function getAllImSources(): DbImSource[] {
  const stmt = db.prepare('SELECT * FROM im_sources ORDER BY created_at DESC');
  return stmt.all() as DbImSource[];
}

export function getImSource(id: string): DbImSource | undefined {
  const stmt = db.prepare('SELECT * FROM im_sources WHERE id = ?');
  return stmt.get(id) as DbImSource | undefined;
}

export function createImSource(source: DbImSource): DbImSource {
  const stmt = db.prepare(`
...
export function updateImSource(id: string, updates: Partial<Pick<DbImSource, 'name' | 'type' | 'config' | 'enabled' | 'last_sync_at'>>): boolean {
  const fields: string[] = [];
  const values: any[] = [];
...
export function deleteImSource(id: string): boolean {
...
export function getConversationsBySource(sourceId: string): DbSourceConversation[] {
  const stmt = db.prepare('SELECT * FROM im_source_conversations WHERE source_id = ? ORDER BY created_at');
  return stmt.all(sourceId) as DbSourceConversation[];
}

export function addConversationToSource(sourceId: string, convId: string, name?: string): DbSourceConversation | null {
  try {
...
export function removeConversationFromSource(sourceId: string, convId: string): boolean {
...
export function getChatRecords(options?: {
  ...
  limit?: number;
  offset?: number;
}): { records: DbChatRecord[]; total: number } {
  ...
  const stmt = db.prepare(
    `SELECT * FROM im_chat_records ${whereClause} ORDER BY timestamp DESC LIMIT ? OFFSET ?`
  );
  const records = stmt.all(...values, limit, offset) as DbChatRecord[];
...
export function getChatRecordsByDateRange(startDate: string, endDate: string): DbChatRecord[] {
  ...
  if (startDate.length === 10 && !startDate.includes('T')) {
    const stmt = db.prepare(
      'SELECT * FROM im_chat_records WHERE timestamp LIKE ? ORDER BY timestamp ASC'
    );
    return stmt.all(`${startDate}%`) as DbChatRecord[];
  }

  // 否则使用范围查询
  const stmt = db.prepare(
    'SELECT * FROM im_chat_records WHERE timestamp >= ? AND timestamp <= ? ORDER BY timestamp ASC'
  );
  return stmt.all(startDate, endDate) as DbChatRecord[];
}

export function importChatRecords(records: DbChatRecord[]): { inserted: number; skipped: number } {
  ...
  const insertMany = db.transaction((recs: DbChatRecord[]) => {
    let inserted = 0;
    let skipped = 0;
    for (const rec of recs) {
      const result = stmt.run(
        rec.id, rec.source_id, rec.im_message_id, rec.sender_name, rec.sender_id,
        rec.group_name, rec.group_id, rec.content, rec.message_type, rec.timestamp,
        rec.is_mentioned, rec.raw_data, rec.synced_at
      );
      if (result.changes > 0) {
        inserted++;
      } else {
...
export function deleteChatRecordsBySource(sourceId: string): number {
...
export function getAnalysisReports(options?: {
  ...
  const stmt = db.prepare(
    `SELECT * FROM analysis_reports ${whereClause} ORDER BY report_date DESC LIMIT ?`
  );
  return stmt.all(...values, limit) as DbAnalysisReport[];
}

export function getAnalysisReportByDate(date: string): DbAnalysisReport | undefined {
  const stmt = db.prepare('SELECT * FROM analysis_reports WHERE report_date = ?');
  return stmt.get(date) as DbAnalysisReport | undefined;
}

export function createAnalysisReport(report: DbAnalysisReport): DbAnalysisReport {
  const stmt = db.prepare(`
  ...
  `);
  stmt.run(
    report.id, report.report_date, report.summary, report.work_priorities,
    report.completed_tasks, report.pending_tasks, report.key_decisions,
    report.follow_ups, report.meeting_notes, report.statistics,
    report.raw_chat_count, report.important_chat_count, report.created_at
  );
  return report;
}
...
export function getAllScheduledTasks(): DbScheduledTask[] {
  const stmt = db.prepare('SELECT * FROM scheduled_tasks ORDER BY created_at DESC');
  return stmt.all() as DbScheduledTask[];
}

export function getScheduledTask(id: string): DbScheduledTask | undefined {
  const stmt = db.prepare('SELECT * FROM scheduled_tasks WHERE id = ?');
  return stmt.get(id) as DbScheduledTask | undefined;
}
...
function normalizeDateTimeRange(startDate?: string, endDate?: string): { start?: string; end?: string } {
...
export function getChatStats(startDate?: string, endDate?: string) {
  const { start, end } = normalizeDateTimeRange(startDate, endDate);
  const conditions: string[] = [];
  const values: any[] = [];
  if (start) { conditions.push('timestamp >= ?'); values.push(start); }
  if (end) { conditions.push('timestamp <= ?'); values.push(end); }
  const whereClause = conditions.length > 0 ? `WHERE ${conditions.join(' AND ')}` : '';
...
export function getTopSenders(startDate?: string, endDate?: string, limit: number = 10) {
  const { start, end } = normalizeDateTimeRange(startDate, endDate);
  const conditions: string[] = [];
  const values: any[] = [];
  if (start) { conditions.push('timestamp >= ?'); values.push(start); }
  ...
  const stmt = db.prepare(`
    ...
    LIMIT ?
  `);
  return stmt.all(...values, limit) as { sender_name: string; message_count: number }[];
}

export function getTopGroups(startDate?: string, endDate?: string, limit: number = 10) {
  const { start, end } = normalizeDateTimeRange(startDate, endDate);
  const conditions: string[] = ['group_name IS NOT NULL'];
  const values: any[] = [];
  if (start) { conditions.push('timestamp >= ?'); values.push(start); }
  if (end) { conditions.push('timestamp <= ?'); values.push(end); }
  ...
  const stmt = db.prepare(`
    ...
    LIMIT ?
  `);
  return stmt.all(...values, limit) as { group_name: string; message_count: number }[];
}

// 修复消息中的 @提及 标记
export function fixMentioned(myName: string): { total: number; fixed: number } {
  const allRecords = db.prepare(`SELECT id, content, is_mentioned FROM im_chat_records`).all() as { id: string; content: string; is_mentioned: number }[];
...
export function saveMeetingIntelligence(meetingId: string, title: string | undefined, startTime: string | undefined, result: object): { id: string } {
...
export function getMeetingIntelligence(meetingId: string): { id: string; meeting_id: string; title: string; start_time: string; result: string; created_at: string; updated_at: string } | undefined {
  return db.prepare(`SELECT * FROM meeting_intelligence WHERE meeting_id = ? ORDER BY created_at DESC LIMIT 1`).get(meetingId) as any;
}

export function deleteMeetingIntelligence(meetingId: string): boolean {
...
// 获取某日的计划（含任务列表）
export function getDailyPlan(date: string): { plan: DbDailyPlan | null; tasks: DbDailyPlanTask[] } {
  const plan = db.prepare('SELECT * FROM daily_plans WHERE date = ?').get(date) as DbDailyPlan | undefined;
  const tasks = db.prepare('SELECT * FROM daily_plan_tasks WHERE plan_date = ? ORDER BY sort_order ASC, created_at ASC').all(date) as DbDailyPlanTask[];
  return { plan: plan || null, tasks };
}

// 获取多日的计划（用于历史未完成任务收集）
export function getDailyPlans(options?: { startDate?: string; endDate?: string; limit?: number }): { plan: DbDailyPlan; tasks: DbDailyPlanTask[] }[] {
  const conditions: string[] = [];
  const values: any[] = [];
  if (options?.startDate) { conditions.push('date >= ?'); values.push(options.startDate); }
  if (options?.endDate) { conditions.push('date <= ?'); values.push(options.endDate); }
  const whereClause = conditions.length > 0 ? `WHERE ${conditions.join(' AND ')}` : '';
  const limit = options?.limit || 30;
  const plans = db.prepare(`SELECT * FROM daily_plans ${whereClause} ORDER BY date DESC LIMIT ?`).all(...values, limit) as DbDailyPlan[];

  return plans.map(plan => ({
    plan,
    tasks: db.prepare('SELECT * FROM daily_plan_tasks WHERE plan_date = ? ORDER BY sort_order ASC, created_at ASC').all(plan.date) as DbDailyPlanTask[],
  }));
}

// 保存/更新整日计划（upsert：有则更新，无则创建）
export function upsertDailyPlan(date: string, goalText: string, tasks: Omit<DbDailyPlanTask, 'plan_date' | 'created_at' | 'updated_at'>[]): { plan: DbDailyPlan | null; tasks: DbDailyPlanTask[] } {
  const now = new Date().toLocaleString('sv-SE', { timeZone: 'Asia/Shanghai' }).replace(' ', 'T');
  ...
  for (let i = 0; i < tasks.length; i++) {
    const t = tasks[i];
    insertStmt.run(
      t.id, date, t.title, t.status, t.priority,
      t.assignee || null, t.estimate_minutes || null, t.notes || null,
      t.completed_at || null, t.expected_completion_at || null,
      t.source_date || null, i, now, now
    );
  }
...
// 更新单个任务
export function updateDailyPlanTask(taskId: string, updates: Partial<Pick<DbDailyPlanTask, 'title' | 'status' | 'priority' | 'assignee' | 'estimate_minutes' | 'notes' | 'completed_at' | 'expected_completion_at'>>): boolean {
  const fields: string[] = [];
  const values: any[] = [];
  for (const [key, value] of Object.entries(updates)) {
    fields.push(`${key} = ?`);
...
// 删除单个任务
export function deleteDailyPlanTask(taskId: string): boolean {
  const result = db.prepare('DELETE FROM daily_plan_tasks WHERE id = ?').run(taskId);
  return result.changes > 0;
}

// 获取历史未完成任务（跨日期，排除已完成/废弃的）
export function getUnfinishedDailyTasks(excludeDate: string, limitDays = 30): DbDailyPlanTask[] {
  const unfinished: DbDailyPlanTask[] = [];
  for (let i = 1; i <= limitDays; i++) {
    const d = new Date();
    d.setDate(d.getDate() - i);
    const dateKey = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
    if (dateKey === excludeDate) continue;
    const tasks = db.prepare(
      "SELECT * FROM daily_plan_tasks WHERE plan_date = ? AND status NOT IN ('completed', 'abandoned') ORDER BY created_at ASC"
    ).all(dateKey) as DbDailyPlanTask[];
...
export function getProjects(options?: {
  ...
  const stmt = db.prepare(`
    ...
    LIMIT ? OFFSET ?
  `);
  const projects = stmt.all(...values, limit, offset) as DbProject[];
...
export function updateProjectPhase(
  ...
  // 获取当前阶段
  const current = db.prepare('SELECT current_phase FROM project_initiations WHERE id = ?').get(projectId) as { current_phase: string } | undefined;
...
// 更新项目阶段进度
export function updateProjectPhaseStatus(
  projectId: string,
  phaseStatus: { initiation?: number; requirement?: number; planning?: number; execution?: number; delivery?: number }
): boolean {
  const current = db.prepare('SELECT phase_status FROM project_initiations WHERE id = ?').get(projectId) as { phase_status: string } | undefined;
  if (!current) return false;
...
// 获取项目成员
export function getProjectMembers(projectId: string): (DbProjectMember & { employee_name?: string; employee_avatar?: string })[] {
  return db.prepare(`
...
// 添加项目成员
export function addProjectMember(projectId: string, employeeId: string, role: string = 'member'): DbProjectMember | null {
  try {
    const id = uuidv4();
    const now = new Date().toLocaleString('sv-SE', { timeZone: 'Asia/Shanghai' }).replace(' ', 'T');
    ...
    return { id, project_id: projectId, employee_id: employeeId, role: role as any, joined_at: now, status: 'active', work_hours_contributed: 0 };
  } catch {
    return null;
  }
}
...
export function syncProjectStatus(projectId: string): { updated: boolean; newPhase: string; newPhaseStatus: Record<string, number>; reason: string } | null {
  try {
    const project = db.prepare('SELECT current_phase, phase_status, status FROM project_initiations WHERE id = ?').get(projectId) as { current_phase: string; phase_status: string; status: string } | undefined;
    ...
    const newPhaseStatus: Record<string, number> = {
    ...
    // 检测风险：已延期但未完成
    const deadlineRow = db.prepare('SELECT deadline FROM project_initiations WHERE id = ?').get(projectId) as { deadline: string | null } | undefined;
    ...
    const now = new Date().toLocaleString('sv-SE', { timeZone: 'Asia/Shanghai' }).replace(' ', 'T');
    const updates: string[] = ['phase_status = ?', 'current_phase = ?', 'updated_at = ?', 'last_synced_at = ?'];
    const values: any[] = [JSON.stringify(newPhaseStatus), newPhase, now, now];
    if (riskLevel) {
    ...
    if (newPhase !== project.current_phase) {
      ...
      `).run(
        ...
        newPhase,
        'system_sync',
        `自动同步：基于 ${done}/${total} 任务完成，execution=${Math.round(executionProgress * 100)}%`,
        now
      );
    }
...
// 批量同步所有活跃项目
export function syncAllProjectStatuses(): { projectId: string; result: NonNullable<ReturnType<typeof syncProjectStatus>> }[] {
  ...
  const results: { projectId: string; result: NonNullable<ReturnType<typeof syncProjectStatus>> }[] = [];
...
// 获取阶段流转日志
export function getProjectPhaseLogs(projectId: string): DbProjectPhaseLog[] {
  return db.prepare(`
    SELECT * FROM project_phase_logs 
    WHERE project_id = ? 
    ORDER BY created_at DESC
  `).all(projectId) as DbProjectPhaseLog[];
}
...
export function getProjectStakeholders(projectId: string): DbProjectStakeholder[] {
  return db.prepare(`
    SELECT * FROM project_stakeholders 
    WHERE project_id = ? 
    ORDER BY created_at ASC
  `).all(projectId) as DbProjectStakeholder[];
}
...
export function removeProjectStakeholder(projectId: string, personName: string): boolean {
  try {
...
export function pushWorkflowOutput(
  source: string,
  content: string,
  type: string = 'text',
  title?: string,
  metadata?: Record<string, any>
): DbWorkflowOutput | null {
  try {
    ...
    const row = db.prepare('SELECT * FROM workflow_outputs WHERE id = ?').get(result.lastInsertRowid) as DbWorkflowOutput;
    return row;
  } catch (e) {
...
export function getWorkflowOutputs(options?: {
  ...
  return db.prepare(sql).all(...params) as DbWorkflowOutput[];
}

export function getWorkflowOutputById(id: number): DbWorkflowOutput | undefined {
  return db.prepare('SELECT * FROM workflow_outputs WHERE id = ?').get(id) as DbWorkflowOutput | undefined;
}

export function deleteWorkflowOutput(id: number): boolean {
  try {
