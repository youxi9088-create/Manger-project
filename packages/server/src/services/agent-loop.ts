...
export interface AttachmentInfo {
  ...
  size: number;
  base64: string;     // base64 编码内容（不含 data: 前缀）
}
...
function buildUserContent(text: string, attachments?: AttachmentInfo[]): AgentMessage["content"] {
  ...
  if (imageAttachments.length === 0) {
    ...
    if (docAttachments.length > 0) {
      enhancedText += "\n\n---\n**附件：**\n" + docAttachments.map(a =>
        `- **${a.name}** (${(a.size / 1024).toFixed(1)}KB, ${a.type})`
      ).join("\n");
    }
    return enhancedText;
  }
...
function moonshotRequest(body: Record<string, unknown>): Promise<Record<string, unknown>> {
  return new Promise((resolve, reject) => {
    const apiKey = process.env.MOONSHOT_API_KEY;
    if (!apiKey) { reject(new Error("未配置 MOONSHOT_API_KEY")); return; }
    ...
    const req = https.request(options, (res) => {
      ...
      res.on("end", () => {
        ...
        catch { reject(new Error(`Moonshot 响应解析失败: ${data.slice(0, 200)}`)); }
      });
    });
    req.on("error", reject);
    req.write(payload);
...
export async function runAgentLoop(
  ...
  for (let i = 0; i < maxIterations; i++) {
    ...
    const response = await moonshotRequest(requestBody) as {
      choices?: Array<{
        message?: {
          role: string;
          content: string | null;
    ...
    if (!choice) throw new Error("Moonshot 返回空响应");

    const msg = choice.message;
    if (!msg) throw new Error("Moonshot 返回空 message");

    // 把当前 assistant 消息加入历史
    loopMessages.push({
      role: "assistant",
      content: msg.content ?? "",
      tool_calls: msg.tool_calls as AgentMessage["tool_calls"],
    });
    ...
    if (!msg.tool_calls || msg.tool_calls.length === 0) {
      ...
      if (fullResponse) onEvent?.({ type: "text", content: fullResponse });
      break;
    }
    ...
    for (const toolCall of msg.tool_calls) {
      ...
      try { params = JSON.parse(toolCall.function.arguments) as Record<string, unknown>; }
      catch { params = {}; }
  ...
  if (!fullResponse) {
    ...
    const toolResults = loopMessages
      ...
      .map(m => {
        try { return typeof m.content === 'string' ? JSON.parse(m.content) : m.content; } catch { return m.content; }
      });
