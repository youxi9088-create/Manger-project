...
export async function transcribeMeetingAudio(filePath: string, options: { audioUrl?: string } = {}) {
  ...
  if (!response.ok) throw new Error(`语音识别服务返回 ${response.status}: ${body.slice(0, 300)}`);
  ...
  if (!transcript) throw new Error('语音识别服务未返回转写文本');
  return transcript;
}

export async function generateMeetingMinutes(transcript: string, meta: { title?: string; startTime?: string }) {
  const apiKey = requiredEnv('MOONSHOT_API_KEY', 'LLM_API_KEY', 'OPENAI_API_KEY');
  if (!apiKey) throw new Error('未配置纪要生成服务。请配置 MOONSHOT_API_KEY、LLM_API_KEY 或 OPENAI_API_KEY。');
  ...
  const body = await response.text();
  if (!response.ok) throw new Error(`纪要生成服务返回 ${response.status}: ${body.slice(0, 300)}`);
  ...
  try {
    ...
    if (!match) throw new Error('纪要生成服务未返回结构化纪要');
  ...
  return {
    ...
    todos: (Array.isArray(minutes?.todos) ? minutes.todos : [])
      .map((todo: unknown) => typeof todo === 'string' ? todo.trim() : '')
      .filter(Boolean),
  };
}
