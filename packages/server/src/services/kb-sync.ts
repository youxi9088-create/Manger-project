...
const KNOWLEDGE_BASE = path.resolve(
  process.env.KNOWLEDGE_BASE_PATH || 'C:/Users/986916/.hermes/knowledge-base'
);
const FEISHU_DOCS_DIR = path.resolve(rootDir, 'data', 'feishu-docs');

// 文档 ID → 知识库路径 映射（可扩展，优先从环境变量读取）
function getDocMapping(): Record<string, { project: string; type: string }> {
...
function blocksToMarkdown(structured: any, rawContent: string, meta: any): string {
  const lines: string[] = [];
...
// ===== 从文档内容中提取关键摘要 =====
function extractSummary(rawContent: string, meta: any) {
  const summary: any = {
...
// ===== 同步单个文档到知识库 =====
export function syncDocument(docId: string): boolean {
  ...
  switch (mapping.type) {
    case 'project-report': {
      ...
      if (newWeekly.length > 0 && Array.isArray(summaryData.weekly_versions)) {
        const existingWeeks = new Set(summaryData.weekly_versions.map((w: any) => w.week));
        ...
        summaryData.weekly_versions.sort((a: any, b: any) => (a.week || '').localeCompare(b.week || ''));
      }
      ...
      while ((rm = riskRegex2.exec(raw)) !== null) {
        if (!risks.find((r: any) => r.description.includes(rm![1].substring(0, 20)))) {
          risks.push({ level: '高', description: rm[1].trim() });
        }
      }
      if (risks.length > 0) summaryData.risks = risks;
...
// ===== 同步全部已拉取的文档 =====
export function syncAll(): { success: number; failed: number; details: string[] } {
...
// ===== 获取所有已配置的文档映射 =====
export function getDocMappings(): Record<string, { project: string; type: string }> {
  return getDocMapping();
}

// ===== 更新文档映射（运行时动态配置）=====
export function updateDocMapping(docId: string, mapping: { project: string; type: string }): void {
