...
export function createProjectInitiation(input: Record<string, unknown>): Record<string, unknown> {
  ...
  dbInstance.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    values.id, values.type, values.status, values.title, values.applicant,
    values.project_leader, values.department, values.project_type, values.demand_source,
    values.demand_date, values.from_pool, values.raw_requirement, values.ai_generated_content,
    values.created_at, values.updated_at
  );

  return dbInstance.prepare("SELECT * FROM project_initiations WHERE id = ?").get(id) as Record<string, unknown>;
}
...
export function updateProjectInitiation(id: string, input: Record<string, unknown>): Record<string, unknown> | null {
...
export function submitProjectInitiation(id: string): Record<string, unknown> | null {
  const existing = dbInstance.prepare("SELECT * FROM project_initiations WHERE id = ?").get(id) as { status: string } | undefined;
  if (!existing) return null;
...
export function deleteProjectInitiation(id: string): boolean {
...
export function transitionProjectPhase(
  ...
  reason?: string,
  triggeredBy?: string
): { success: boolean; newPhase?: string; error?: string } {
  const actionToPhase: Record<string, string> = {
    submit: "submitted",
    approve: "approved",
  ...
  if (!targetPhase) return { success: false, error: "无效的操作类型" };

  const ok = updateProjectPhase(id, targetPhase, triggeredBy, reason);
  if (!ok) return { success: false, error: "项目不存在或状态未变更" };
