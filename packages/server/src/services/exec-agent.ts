...
export async function runExecAgent(options: ExecAgentOptions): Promise<ExecAgentResult> {
  const taskId = uuidv4();
  ...
  let toolCallCount = 0;
  const toolCallsRecord: Array<{ name: string; params: unknown; result: unknown }> = [];
  ...
  try {
    ...
    `).run(
      summary,
      JSON.stringify(toolCallsRecord),
      toolCallCount,
