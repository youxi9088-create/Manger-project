...
export interface VersionRiskResult {
  ...
  stats: {
    ...
    done_rate: number;           // 0-1
    days_left: number | null;
    total_estimated_hours: number;
    remaining_hours: number;     // 未完成任务的工时
    estimated_finish_days: number | null; // 按每天 8h 预估还需几个工作日
  };
}
 ...
 */
export function computeVersionRisk(versionId: string): VersionRiskResult | null {
  const version = dbInstance.prepare(
    `SELECT id, status, goals, expected_release_date FROM versions WHERE id = ?`
  ).get(versionId) as VersionRow | undefined;

  if (!version) return null;

  // 解析目标
  let goals: Array<{ id: string; title: string; requirement_id: string | null }> = [];
  try { goals = JSON.parse(version.goals || '[]'); } catch { goals = []; }
  const totalGoals = goals.length;
  const convertedGoals = goals.filter(g => g.requirement_id).length;

  // 该版本下的需求
  const requirements = dbInstance.prepare(
    `SELECT id, version_id, status FROM requirement_analyses WHERE version_id = ?`
  ).all(versionId) as RequirementRow[];
  ...
  if (requirements.length > 0) {
    ...
    tasks = dbInstance.prepare(
      `SELECT id, status, requirement_id, estimated_hours FROM dev_tasks WHERE requirement_id IN (${placeholders})`
    ).all(...requirements.map(r => r.id)) as TaskRow[];
  }
  ...
  const promote = (to: RiskLevel) => {
    const order: RiskLevel[] = ['safe', 'warning', 'critical', 'overdue'];
  ...
  if (isDelayed && level === 'safe') {
    level = 'overdue';
    if (reasons.length === 0) reasons.push('版本被手动标记为延期');
    if (strategies.length === 0) strategies.push('建议评估延期原因并制定恢复计划');
  }
  ...
  const result: VersionRiskResult = {
  ...
  try {
    ...
    dbInstance.prepare(
      `UPDATE versions SET computed_risk = ?, risk_reasons = ?, risk_level = ?, last_check_at = datetime('now','localtime') WHERE id = ?`
    ).run(level, JSON.stringify(reasonsWithStrategies), mappedRiskLevel, versionId);
  } catch { /* 写回失败不影响返回 */ }
...
/** 批量重算所有版本预警 */
export function recomputeAllVersionRisks(): { total: number; ok: number } {
  const rows = dbInstance.prepare(`SELECT id FROM versions`).all() as Array<{ id: string }>;
  let ok = 0;
  for (const r of rows) {
    try { computeVersionRisk(r.id); ok++; } catch { /* ignore */ }
  }
