...
import { initAgentTools, getAllTools, type AgentTool } from "../services/agent-tools.js";
...
function toMcpTool(tool: AgentTool): Tool {
...
export function createMcpServer() {
  ...
  const exposedTools = allTools.filter((t) => {
    ...
    if (DELETE_TOOL_NAMES.has(name) && !ALLOW_DELETE) return false;
    if (READONLY_MODE) return READONLY_TOOL_NAMES.has(name);
    return true;
  });
