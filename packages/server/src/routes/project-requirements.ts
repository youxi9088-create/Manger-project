...
router.get("/api/projects/:id/requirements", (req, res) => {
  try {
    ...
    const rows = dbInstance.prepare(
      `SELECT * FROM requirement_analyses 
       WHERE initiation_id = ? OR project_id = ? 
       ORDER BY created_at DESC`
    ).all(id, id);
    res.json({ success: true, data: (rows as any[]).map(enrichRequirement) });
  } catch (error: any) {
...
router.post("/api/projects/:id/requirements", (req, res) => {
  try {
    ...
    const lastRow = dbInstance.prepare(
    ...
    dbInstance.prepare(
      `INSERT INTO requirement_analyses (id, initiation_id, project_id, title, input_type, raw_input, status, created_at, updated_at)
       VALUES (?, ?, ?, ?, 'text', ?, 'draft', ?, ?)`
    ).run(reqId, id, id, title, raw_input || null, ts, ts);

    const row = dbInstance.prepare("SELECT * FROM requirement_analyses WHERE id = ?").get(reqId);
    res.json({ success: true, data: enrichRequirement(row as any) });
  } catch (error: any) {
