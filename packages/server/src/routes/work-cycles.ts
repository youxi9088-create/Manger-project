...
function generateWorkCycleId(): string {
  const now = new Date();
...
function completeActiveCycle(employeeId: string): void {
  const activeCycle = dbInstance.prepare(
    `SELECT id FROM work_cycles WHERE employee_id = ? AND status = 'active' ORDER BY start_time DESC LIMIT 1`
  ).get(employeeId) as { id: string } | undefined;

  if (activeCycle) {
    dbInstance.prepare(
      `UPDATE work_cycles SET status = 'completed', end_time = datetime('now','localtime'), progress = 100, updated_at = datetime('now','localtime') WHERE id = ?`
    ).run(activeCycle.id);
  }
}
...
router.get("/api/work-cycles", (req, res) => {
  try {
    ...
    const stmt = dbInstance.prepare(
      `SELECT wc.*, COALESCE(dt.estimated_hours, wc.estimated_hours) as estimated_hours
       FROM work_cycles wc
...
router.post("/api/work-cycles", (req, res) => {
  try {
    ...
    if (!employee_id) return res.status(400).json({ error: "employee_id 不能为空" });
    ...
    dbInstance.prepare(
      `INSERT INTO work_cycles (id, employee_id, task_id, task_title, start_time, estimated_hours, status, progress, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, 'active', 0, ?, ?)`
    ).run(id, employee_id, task_id || null, task_title || null, now, estimated_hours || null, now, now);
...
router.patch("/api/work-cycles/:id", (req, res) => {
  try {
    ...
    if (!existing) return res.status(404).json({ error: "工作周期不存在" });
    ...
    if (req.body.status === "completed") {
      const record = existing as { end_time: string | null; employee_id: string };
      if (!record.end_time) {
        dbInstance.prepare(
          `UPDATE work_cycles SET end_time = datetime('now','localtime') WHERE id = ?`
        ).run(req.params.id);
      }
      dbInstance.prepare(
        `UPDATE employees SET status = 'available', updated_at = datetime('now','localtime') WHERE id = ?`
      ).run(record.employee_id);
    }
...
router.post("/api/work-cycles/:id/complete", (req, res) => {
  try {
    const cycle = dbInstance.prepare("SELECT * FROM work_cycles WHERE id = ?").get(req.params.id) as
      { id: string; employee_id: string; status: string } | undefined;
    if (!cycle) return res.status(404).json({ error: "工作周期不存在" });
    if (cycle.status !== "active") return res.status(400).json({ error: "只能结束进行中的工作周期" });
  ...
  try {
    ...
    if (activeCycle) {
      ...
      dbInstance.prepare(
        `INSERT INTO work_cycles (id, employee_id, task_id, task_title, start_time, estimated_hours, status, progress, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, 'queued', 0, ?, ?)`
      ).run(cycleId, employeeId, task_id || null, task_title || null, now, estimated_hours || null, now, now);
...
router.post("/api/employees/:id/complete-work", (req, res) => {
  try {
    ...
    const nextQueued = dbInstance.prepare(
      `SELECT id, task_id, task_title, estimated_hours FROM work_cycles WHERE employee_id = ? AND status = 'queued' ORDER BY created_at ASC LIMIT 1`
    ).get(employeeId) as { id: string; task_id: string | null; task_title: string | null; estimated_hours: number | null } | undefined;

    let autoStarted: typeof nextQueued | null = null;

    if (nextQueued) {
      // 自动启动排队中的下一个任务
      const now = new Date().toISOString();
      dbInstance.prepare(
        `UPDATE work_cycles SET status = 'active', start_time = ?, updated_at = ? WHERE id = ?`
      ).run(now, now, nextQueued.id);
      // 员工保持 busy
      dbInstance.prepare("UPDATE employees SET status = 'busy', updated_at = datetime('now','localtime') WHERE id = ?").run(employeeId);
      autoStarted = dbInstance.prepare("SELECT * FROM work_cycles WHERE id = ?").get(nextQueued.id) as typeof nextQueued;
    } else {
      // 没有排队任务 → 员工回到可用状态
      dbInstance.prepare(
        "UPDATE employees SET status = 'available', updated_at = datetime('now','localtime') WHERE id = ?"
...
router.delete("/api/work-cycles/:id", (req, res) => {
  try {
    ...
    const record = existing as { status: string; employee_id: string };
