...
import {
  deleteRemoteMeetingIntelligence,
  loadRemoteMeetingIntelligence,
  saveRemoteMeetingIntelligence,
} from "../services/meeting-intelligence-store.js";
import {
  loadMeetingIntelligenceJob,
  saveMeetingIntelligenceJob,
  type MeetingIntelligenceJob,
} from "../services/meeting-intelligence-job-store.js";
import {
  loadMeetingProcessJob,
  saveMeetingProcessJob,
  type MeetingProcessJob,
} from "../services/meeting-process-job-store.js";
import { waitUntil } from "../utils/fn-functions.js";

const router = Router();

function publicMeetingProcessJob(job: MeetingProcessJob) {
...
async function updateMeetingProcessJob(job: MeetingProcessJob, update: Partial<MeetingProcessJob>) {
  Object.assign(job, update, { updatedAt: new Date().toISOString() });
  await saveMeetingProcessJob(job);
}

async function runMeetingProcessJob(job: MeetingProcessJob, directUrl?: string, options?: { forceRefresh?: boolean }) {
  try {
    ...
    if (stat.size < 10_000) throw new Error(`会议音频下载不完整（${stat.size} 字节）`);
...
router.get("/api/meetings/download", async (req, res) => {
  ...
  const format = (String(req.query.format || 'audio') === 'video' ? 'video' : 'audio') as 'audio' | 'video';
  if (!meetingId) return res.status(400).json({ success: false, error: 'missing meetingId' });
...
router.get("/api/meetings/process/:jobId", async (req, res) => {
  ...
  if (!job) return res.status(404).json({ success: false, error: '转录任务不存在或服务已重启，请重新发起转录' });
...
router.post("/api/meetings/process", async (req, res) => {
  ...
  try {
    ...
    if (regenerate) {
      if (!transcript) throw new Error('没有可用于重新生成纪要的真实转写文本');
    ...
    const records = await meetingService.fetchMeetingListWithToken();
    const record = records.find((item) => item.meetingId === meetingId);
    if (!record) throw new Error('未在真实会议列表中找到该录制文件，请刷新会议列表后重试');
...
function normalizeLevel(value: unknown, fallback = 'medium') {
  const normalized = text(value).toLowerCase();
  return normalized === 'high' || normalized === 'low' || normalized === 'medium' ? normalized : fallback;
}

function normalizeMeetingIntelligence(value: unknown) {
...
async function generateAndPersistMeetingIntelligence(input: {
  ...
  if (!apiKey) throw new Error('未配置会议分析 API Key');
...
所有事实必须来自转录；无法确认的负责人、日期和决策留空，不要猜测。evidence 只能写简短依据摘要。`;
  ...
  if (!response.ok) throw new Error(`AI 错误: ${response.status} ${await response.text()}`);
  const body = await response.json() as { choices?: Array<{ message?: { content?: string } }> };
  const content = text(body.choices?.[0]?.message?.content);
  const jsonMatch = content.match(/\{[\s\S]*\}/);
  if (!jsonMatch) throw new Error('会议分析服务未返回结构化结果');
...
router.post("/api/meetings/intelligence", async (req, res) => {
  ...
  if (req.body.async === true) {
    ...
    const job: MeetingIntelligenceJob = {
      jobId: crypto.randomUUID(),
...
- owner 字段尽量提取会议中提到的真实人名`;
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        if (!saved) {
          try { saveMeetingIntelligence(intelligenceMeetingId, title || '', startTime || '', parsed); saved = true; }
          catch (e) { console.error('保存本地分析结果失败:', e); }
        }
...
router.get("/api/meetings/intelligence/jobs/:jobId", async (req, res) => {
  try {
    ...
    if (!job) return res.status(404).json({ success: false, error: '会议分析任务不存在' });
