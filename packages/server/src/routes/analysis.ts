...
function isDailyReportFile(name: string): boolean {
  return /^daily_report_\d{4}-\d{2}-\d{2}\.json$/i.test(name);
}

function pickLatestDailyReportFile(): { fileName: string; fullPath: string } | null {
  try {
...
router.get('/api/analysis-reports/latest', (req, res) => {
  try {
    ...
    let reportFile: { fileName: string; fullPath: string } | null = null;
    if (date) {
      const fileName = `daily_report_${date}.json`;
      const fullPath = path.join(REPORTS_ROOT_DIR, fileName);
      if (fs.existsSync(fullPath)) reportFile = { fileName, fullPath };
    } else {
...
router.post("/api/analysis/run", async (req, res) => {
  ...
  try {
    ...
    for await (const msg of sdkSession.stream()) {
      if (msg.type === "assistant") {
        ...
        if (newText.length > lastSentLength) {
          ...
          try { res.write(`data: ${JSON.stringify({ type: "text", content: delta })}\n\n`); } catch { }
        }
    ...
    try {
      ...
      if (backtickMatch) {
        try { analysisResult = JSON.parse(backtickMatch[1].trim().replace(/^\uFEFF/, '')); } catch { }
      }
      // 2. json { ... } 无反引号（AI 常见错误）
      if (!analysisResult) {
        const nakedMatch = fullResponse.match(/^(?:\s*json)?\s*(\{[\s\S]*\})\s*$/i);
        if (nakedMatch) {
          try { analysisResult = JSON.parse(nakedMatch[1].trim()); } catch { }
        }
      }
      // 3. 从响应中提取第一个 { ... } JSON 对象
      if (!analysisResult) {
        ...
        if (firstObjMatch) {
          try { analysisResult = JSON.parse(firstObjMatch[1].trim()); } catch {
            ...
            while (trimmed.length > 1 && !analysisResult) {
              ...
              try { analysisResult = JSON.parse(trimmed); } catch { trimmed = trimmed.slice(0, -1); }
            }
          }
        }
      }
      ...
      if (!analysisResult) {
        ...
        if (balanced) {
          try { analysisResult = JSON.parse(balanced); } catch { }
        }
      }
      if (!analysisResult) analysisResult = parseMarkdownReport(fullResponse);
      ...
      // 收集所有真实群名，用于后处理校验
      const realGroupNames = [...new Set(chatRecords.map((r) => r.group_name).filter((n): n is string => !!n))];

      // 后处理：用真实群名替换 group_summaries 中的模糊/通用名称
      function normalizeGroupName(given: string): string {
        if (!given) return "";
        // 精确匹配（忽略大小写和空格）
        const exactMatch = realGroupNames.find(
          (rn) => rn.toLowerCase().trim() === given.toLowerCase().trim()
        );
        if (exactMatch) return exactMatch;

        // 包含匹配：真实群名包含 LLM 返回的名称 或反之
        const containMatch = realGroupNames.find(
          (rn) => rn.includes(given) || given.includes(rn)
        );
      ...
      const normalizedGroupSummaries = (analysisResult.group_summaries || []).map((g: any) => ({
      ...
      const normalizedSenderActivities = (analysisResult.sender_activities || []).map((s: any) => ({
      ...
      try { res.write(`data: ${JSON.stringify({ type: "saved" })}\n\n`); } catch { }
    } catch (parseError: any) {
      try { res.write(`data: ${JSON.stringify({ type: "error", message: "分析结果解析失败: " + parseError.message })}\n\n`); } catch { }
    }
  } catch (error: any) {
    try { res.write(`data: ${JSON.stringify({ type: "error", message: error?.message || "分析执行失败" })}\n\n`); } catch { }
  }
  try { res.end(); } catch { }
});
