 ...
 *   done       → 所有任务均为 done
 */
function computeRequirementStatus(req: RequirementRow): string {
  const tasks = dbInstance.prepare(
    "SELECT status FROM dev_tasks WHERE requirement_id = ?"
  ).all(req.id) as DevTaskRow[];
...
function enrichRequirement<T extends RequirementRow>(row: T): T & { computed_status: string } {
  return { ...row, computed_status: computeRequirementStatus(row) };
}

/** 批量附加 computed_status */
function enrichRequirements<T extends RequirementRow>(rows: T[]): (T & { computed_status: string })[] {
...
router.get("/api/requirements", (req, res) => {
  try {
    ...
    if (initiation_id && typeof initiation_id === 'string') { sql += " AND initiation_id = ?"; params.push(initiation_id); }
    if (version_id && typeof version_id === 'string') { sql += " AND version_id = ?"; params.push(version_id); }
    if (project_id && typeof project_id === 'string') { sql += " AND project_id = ?"; params.push(project_id); }
    sql += " ORDER BY created_at DESC";
    const rows = dbInstance.prepare(sql).all(...params) as RequirementRow[];
    res.json({ success: true, data: enrichRequirements(rows) });
  } catch (error: any) { res.status(500).json({ error: error?.message }); } // eslint-disable-next-line @typescript-eslint/no-explicit-any
});

router.get("/api/requirements/:id", (req, res) => {
  try {
    const row = dbInstance.prepare("SELECT * FROM requirement_analyses WHERE id = ?").get(req.params.id) as RequirementRow | undefined;
...
router.patch("/api/requirements/:id", (req, res) => {
  try {
    ...
    const sets: string[] = []; const values: any[] = []; // eslint-disable-next-line @typescript-eslint/no-explicit-any
    for (const f of allowed) { if (req.body[f] !== undefined) { sets.push(`${f} = ?`); values.push(req.body[f]); } }
    if (sets.length === 0) return res.status(400).json({ error: "没有需要更新的字段" });
    sets.push("updated_at = ?"); values.push(nowStr()); values.push(req.params.id);
    dbInstance.prepare(`UPDATE requirement_analyses SET ${sets.join(', ')} WHERE id = ?`).run(...values);
    const row = dbInstance.prepare("SELECT * FROM requirement_analyses WHERE id = ?").get(req.params.id) as RequirementRow;
    res.json({ success: true, data: enrichRequirement(row) });
  } catch (error: any) { res.status(500).json({ error: error?.message }); }
});
...
router.delete("/api/requirements/:id", (req, res) => {
  try {
    ...
    if (result.changes === 0) return res.status(404).json({ error: "需求不存在" });
...
router.get("/api/dev-tasks", (req, res) => {
  try {
    ...
    const params: string[] = [];
    if (requirement_id && typeof requirement_id === 'string') { sql += " AND requirement_id = ?"; params.push(requirement_id); }
    if (status && typeof status === 'string') { sql += " AND status = ?"; params.push(status); }
...
router.post("/api/dev-tasks", (req, res) => {
  try {
    ...
    if (!requirement_id || !title) return res.status(400).json({ error: "requirement_id 和 title 必填" });
    const ts = nowStr();
    dbInstance.prepare(`
...
router.patch("/api/dev-tasks/:id", (req, res) => {
  try {
    ...
    if (req.body.estimated_hours !== undefined) {
      ...
      dbInstance.prepare(
        `UPDATE work_cycles SET estimated_hours = ?, updated_at = datetime('now','localtime') WHERE task_id = ? AND status IN ('active', 'queued')`
      ).run(req.body.estimated_hours, req.params.id);

      // 触发关联版本的预警重算
      const task = dbInstance.prepare(
        `SELECT requirement_id FROM dev_tasks WHERE id = ?`
      ).get(req.params.id) as { requirement_id: string | null } | undefined;
      if (task?.requirement_id) {
        const reqRow = dbInstance.prepare(
          `SELECT version_id FROM requirement_analyses WHERE id = ?`
        ).get(task.requirement_id) as { version_id: string | null } | undefined;
...
router.post("/api/requirements/:id/analyze", async (req, res) => {
  const existing = dbInstance.prepare("SELECT * FROM requirement_analyses WHERE id = ?").get(req.params.id) as any; // eslint-disable-next-line @typescript-eslint/no-explicit-any
  if (!existing) return res.status(404).json({ error: "需求不存在" });
  if (!existing.raw_input) return res.status(400).json({ error: "请先填写需求描述" });
...
router.post("/api/requirements/:id/generate-tasks", async (req, res) => {
  const existing = dbInstance.prepare("SELECT * FROM requirement_analyses WHERE id = ?").get(req.params.id) as any; // eslint-disable-next-line @typescript-eslint/no-explicit-any
  if (!existing) return res.status(404).json({ error: "需求不存在" });
  if (!existing.ai_analysis) return res.status(400).json({ error: "请先完成需求分析" });
...
- 工时以小时为单位，务实估算（考虑 UE5 开发实际复杂度）`;
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        const insertStmt = dbInstance.prepare(`
          ...
          VALUES (?, ?, ?, ?, ?, ?, 'todo', ?, ?, ?, ?)
        `);
        const insertMany = dbInstance.transaction((items: any[]) => { // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    } catch (e: any) { send('done', { result: null, raw: fullContent, error: e?.message }); } // eslint-disable-next-line @typescript-eslint/no-explicit-any
  } catch (error: any) { send('error', { message: error?.message || '拆解失败' }); } // eslint-disable-next-line @typescript-eslint/no-explicit-any
  finally { res.end(); }
});
