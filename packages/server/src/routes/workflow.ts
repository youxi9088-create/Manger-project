...
import {
  ...
  getProjectInfoFiles,
  getProjectInfoFileByProid,
  deleteProjectInfoFile,
  syncKnowledgeBaseForProjectInfo,
} from '../services/db.js';
import { loadWorkflowJob, saveWorkflowJob, type WorkflowJob } from '../services/workflow-job-store.js';
...
const AIHUB_APP_IDS: Record<string, string> = {
  'project-info': 'eee4cd05-ded3-4794-84e2-73d2494a5dc6',
};

function getAIHubAppId(alias: string): string {
...
const router = Router();
const recoveringWorkflowJobs = new Set<string>();
...
/* ─── 轮询等待 AI-Hub Run 完成 ─── */
async function pollAIHubRun(runId: string, token: string): Promise<{ status: string; error?: any; aiHubError?: any; lastPollError?: string }> {
...
async function finishWorkflowRun(params: {
  ...
  if (!outputsResp.ok) throw new Error(`获取工作流输出失败: HTTP ${outputsResp.status}`);
  ...
  const rawAnswer = (llmReport ? llmReport : '')
    + (todoText ? '\n\n---\n\n## 执行中待办\n\n' + todoText : '')
    || (outputsJson as any)?.text || (outputsJson as any)?.answer || JSON.stringify(outputsJson, null, 2);
...
function buildWorkflowJobResult(result: Awaited<ReturnType<typeof finishWorkflowRun>>) {
...
async function recoverWorkflowJob(job: WorkflowJob): Promise<WorkflowJob> {
  if (job.status !== 'running' || recoveringWorkflowJobs.has(job.jobId)) return job;
  ...
  try {
    ...
    if (statusJson.status === 'failed') {
      ...
      const failed = { ...job, status: 'failed' as const, updatedAt: new Date().toISOString(), error: message };
      await saveWorkflowJob(failed);
      return failed;
    }
    if (statusJson.status !== 'succeeded') return job;
...
function truncateUtf8(value: string, maxBytes: number): string {
  ...
  for (const character of value) {
    ...
    if (usedBytes + characterBytes + suffixBytes > maxBytes) break;
    result += character;
...
function getInnerOutputs(outputsJson: any): Record<string, any> {
 ...
 */
function extractWorkflowStructuredData(outputsJson: any): Record<string, any> {
      ...
      if (first && typeof first === 'object') {
        ...
        for (const item of value) {
          ...
          for (const [k, v] of Object.entries(item)) {
            if (k === 'status_code' || k === 'code' || k === 'msg' || k === 'message') continue;
...
function deepUnwrap(v: any): any {
  ...
  if (v && typeof v === 'object') {
    ...
    if (keys.includes('status_code')) {
      ...
      if (valueKey !== undefined) return deepUnwrap(v[valueKey]);
    }
    return v;
  }
  return v;
...
function normalizeWorkflowStructuredData(sd: Record<string, any>): Record<string, any> {
  ...
  const mappedSd: Record<string, any> = { ...sd };
  ...
  const norm: Record<string, any> = { ...mappedSd };
  ...
  function extractItems(v: any, fieldName: string): any[] {
    ...
    const collect = (val: any): any[] => {
      if (Array.isArray(val)) {
        ...
        if (val.every(item => item && typeof item === 'object' && !Array.isArray(item))) {
          const allWrappers = val.every(item =>
            Array.isArray(item.items) ||
            Array.isArray(item[fieldName]) ||
            (typeof item[fieldName] === 'string' && /^[\[{]/.test(item[fieldName].trim()))
          );
          if (allWrappers) return val.flatMap(collect);
        }
        return val;
      }
      if (val && typeof val === 'object') {
        if (Array.isArray(val.items)) return val.items;
        if (Array.isArray(val[fieldName])) return val[fieldName];
        if (typeof val[fieldName] === 'string') {
          try { return collect(JSON.parse(val[fieldName])); } catch { return []; }
        }
      }
      return [];
    };
  ...
  if (weeklyVersions.length > 0) {
    norm.weekly_versions = weeklyVersions.map((w: any) => ({
      time: w.time || w.week || '',
  ...
  if (mappedSd.stage_objectives && typeof mappedSd.stage_objectives === 'object') {
    ...
    const normalizeStageItem = (s: any) => ({
      name: s.name || '',
      target: s.target || '',
  ...
  if (monthlyPlan.length > 0) {
    norm.monthly_plan = monthlyPlan.map((m: any) => ({
  ...
  if (coreValuesRaw.length > 0) {
    norm.core_values = coreValuesRaw.flatMap((item: any) => {
...
async function analyzeProjectInfoWithLLM(params: {
  ...
  baseUrl?: string;
  apiKey?: string;
}): Promise<{ analyzed: string; structuredData: Record<string, any>; usedLLM: boolean; error?: string }> {
...
${typeof rawAnswer === 'string' ? rawAnswer.slice(0, 4000) : JSON.stringify(rawAnswer).slice(0, 4000)}`;
  ...
  try {
    ...
    let extractedSummary: Record<string, any> = {};
...
router.get('/api/workflow/jobs/:jobId', async (req, res) => {
  try {
    ...
    if (!job) return res.status(404).json({ success: false, error: '工作流任务不存在' });
    if (job.status === 'running') job = await recoverWorkflowJob(job);
