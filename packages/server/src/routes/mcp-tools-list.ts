...
function buildParamsDoc(schema: any): { params: any[]; required: string[] } {
  if (!schema || !schema.properties) return { params: [], required: [] };
...
router.get("/api/mcp/tools-list", (req, res) => {
  ...
  for (const tool of allTools) {
    ...
    // 跳过 MCP 内部工具
    if (name === "ListTools" || name === "CallTool") continue;
