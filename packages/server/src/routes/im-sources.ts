...
router.put("/api/im/sources/:id", (req, res) => {
  try {
    ...
    if (!success) return res.status(404).json({ error: "数据源不存在" });
...
router.post("/api/im/sources/:id/conversations", (req, res) => {
  try {
    ...
    if (!convId) return res.status(400).json({ error: "会话 ID 不能为空" });
    const conversation = db.addConversationToSource(id, convId, name);
    if (!conversation) return res.status(400).json({ error: "会话已存在或添加失败" });
