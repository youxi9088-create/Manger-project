...
import { PermissionResult, CanUseTool } from "@tencent-ai/agent-sdk";
import * as db from "../services/db.js";
import dbInstance from "../services/db.js";
import { defaultModel, getAuthToken, createAgentSession, resolveModel } from "../utils/agent-sdk.js";
import fs from "fs";
          ...
          if (tc.name === "dispatch_exec_agent" && tc.result?.task_id && (!tc.result?.summary || tc.result.summary === "")) {
            try {
              const task = dbInstance.prepare(
                "SELECT status, result, error, iterations, tool_calls FROM agent_tasks WHERE id = ?"
              ).get(tc.result.task_id) as { status: string; result: string | null; error: string | null; iterations: number; tool_calls: string | null } | undefined;
              if (task) {
                // 优先用 result，其次 error，最后从 tool_calls 提取摘要
                let summary = task.result || task.error || "";
                if (!summary && task.tool_calls) {
                  try {
                    const calls = JSON.parse(task.tool_calls) as Array<{ name: string; result: unknown }>;
                  ...
                  } catch { summary = `[${task.status}] 执行了 ${task.iterations} 轮工具调用`; }
                }
                if (!summary) summary = `[${task.status}] 执行了 ${task.iterations} 轮工具调用`;
...
router.post("/api/chat", async (req, res) => {
  ...
  if (Array.isArray(attachments) && attachments.length > 0) {
    ...
    for (const att of attachments) {
      if (!att.name || !att.type || !att.base64) continue;
      try {
  ...
  try {
    ...
    for await (const msg of sdkSession.stream()) {
      if (msg.type === "system" && (msg as any).subtype === "init") {
        ...
        if (sId && sId !== sdkSessionId) db.updateSession(session.id, { sdk_session_id: sId });
