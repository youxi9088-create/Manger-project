...
function getAuthPath(): string {
  ...
  // 优先用统一路径
  if (fs.existsSync(AUTH_PATH)) return AUTH_PATH;
...
router.post('/execute', async (req, res) => {
  ...
  try {
    ...
    let activeFrame: any = null;

    while (Date.now() - start < timeout && actionState !== "CLICKED") {
      for (const frame of page.frames()) {
        if (actionState === "SEARCHING") {
            const scrollResult = await frame.evaluate((targetUuid: string) => {
                const walker = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT);
                let node;
                while (node = walker.nextNode()) {
                    if (node.textContent?.includes(targetUuid)) {
                        const row = (node as any).parentElement?.closest('tr, .ant-table-row, .fish-table-row, [class*="row"]');
                        if (row) {
        ...
        if (actionState === "SCROLLED" && activeFrame === frame) {
             const clickResult = await frame.evaluate((targetUuid: string) => {
                const walker = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT);
                let node;
                while (node = walker.nextNode()) {
                    if (node.textContent?.includes(targetUuid)) {
                        const row = (node as any).parentElement?.closest('tr, .ant-table-row, .fish-table-row, [class*="row"]');
                        if (row) {
                            let target: any = row.querySelector('[title="@增加子任务链接"]');
                            if (!target) {
                                const candidates = Array.from(row.querySelectorAll('a, span, i, div'));
                                target = candidates.find((el: any) => {
                                    const txt = (el.innerText || "").trim();
                                    return txt.includes('@') && txt.includes('增加') && txt.includes('链接');
                                });
                            }
                            if (!target) {
                                target = Array.from(row.querySelectorAll('a')).find((el: any) => el.href && el.href.includes('input_'));
                            }
                            if (target) {
    ...
    for (const frame of targetPage.frames()) {
        ...
        if (await btn.isVisible()) { addButton = btn; break; }
    }
    ...
    if (taskData['需求名称']) {
         ...
         if (await titleInput.isVisible()) { await titleInput.fill(taskData['需求名称']); }
         else {
             const cell = currentRow.locator('td').nth(2);
             await cell.click({ force: true }); 
             await targetPage.waitForTimeout(500); 
             const cellInput = cell.locator('input, textarea').first();
             if (await cellInput.isVisible()) { await cellInput.fill(taskData['需求名称']); }
             else { await targetPage.keyboard.type(taskData['需求名称']); }
         }
    }
    ...
    if (taskData['负责人']) {
        ...
        if (await trigger.isVisible()) { await trigger.click(); } else { await devCell.click({ force: true }); }
    ...
    let rawDate: any = taskData['完成时间'] || taskData['*编码计划完成时间'] || taskData['编码计划完成时间'];
    if (rawDate) {
        if (/^\d+$/.test(String(rawDate))) rawDate = parseInt(rawDate, 10);
    ...
    try {
        ...
        if (await dateInput.isVisible()) await dateInput.dispatchEvent('click');
        ...
        try {
            ...
            if (await fallbackInput.isVisible()) {
                await fallbackInput.evaluate((el: any) => el.removeAttribute('readonly'));
    ...
    try { await targetPage.screenshot({ path: `error-${uuid}.png`, fullPage: true }); } catch (e) {}
