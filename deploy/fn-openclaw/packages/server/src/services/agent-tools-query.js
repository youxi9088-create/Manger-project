...
export function registerQueryTools(register) {
    // ===== 系统状态 =====
    register({
        definition: {
        ...
        execute: async () => {
            const counts = {
                projects: dbInstance.prepare("SELECT COUNT(*) as n FROM project_initiations").get().n,
                requirements: dbInstance.prepare("SELECT COUNT(*) as n FROM requirement_analyses").get().n,
                dev_tasks: dbInstance.prepare("SELECT COUNT(*) as n FROM dev_tasks").get().n,
                employees: dbInstance.prepare("SELECT COUNT(*) as n FROM employees").get().n,
                versions: dbInstance.prepare("SELECT COUNT(*) as n FROM versions").get().n,
                active_tasks: dbInstance.prepare("SELECT COUNT(*) as n FROM dev_tasks WHERE status != 'done'").get().n,
            };
            return counts;
        },
    });
    ...
    register({
        ...
        execute: async (params) => {
            const row = dbInstance.prepare("SELECT * FROM project_initiations WHERE id = ?").get(params.id);
    ...
    register({
        ...
        execute: async (params) => {
            const row = dbInstance.prepare("SELECT * FROM requirement_analyses WHERE id = ?").get(params.id);
    ...
    register({
        ...
        execute: async (params) => {
            const row = dbInstance.prepare("SELECT * FROM dev_tasks WHERE id = ?").get(params.id);
    ...
    register({
        ...
        execute: async (params) => {
            ...
            if (params.requirement_id) {
                ...
                args.push(params.requirement_id);
            }
            if (params.assignee_id) {
                sql += " AND assignee_id = ?";
                args.push(params.assignee_id);
            }
            sql += " ORDER BY created_at DESC LIMIT ?";
            args.push(String(params.limit ?? 20));
            return dbInstance.prepare(sql).all(...args);
    ...
    register({
        ...
        execute: async (params) => {
            ...
            if (params.is_agent === "true") {
                sql += " AND agent_type IS NOT NULL";
            }
            sql += " ORDER BY created_at DESC LIMIT ?";
    ...
    register({
        ...
        execute: async (params) => {
            ...
            if (params.sender) {
                ...
                args.push(`%${params.sender}%`);
            }
            if (params.start_date) {
                sql += " AND timestamp >= ?";
                args.push(`${params.start_date}T00:00:00`);
            }
            if (params.end_date) {
                sql += " AND timestamp <= ?";
                args.push(`${params.end_date}T23:59:59`);
            }
...
//# sourceMappingURL=agent-tools-query.js.map