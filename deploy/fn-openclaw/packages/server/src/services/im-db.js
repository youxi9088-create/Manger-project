...
export function getDbPath(basePath, employeeId) {
    // 兼容传入 _986916@nd 格式或纯工号 986916 格式
    const normalizedId = employeeId.replace(/^_/, '').replace(/@nd$/, '');
    return path.join(basePath, `_${normalizedId}@nd`);
}
export function openImDatabase(config) {
    ...
    for (const file of dbFiles) {
        ...
        try {
            ...
            console.log(`[IM-DB] Opened ${file}, tables: ${tables.map((t) => t.name).join(', ')}`);
            if (tables.some((t) => t.name === 'messages' || t.name === 'Buddies' || t.name === 'Message' || t.name === 'Roster' || t.name === 'Chat')) {
...
export function getMessageTables(db) {
    const tables = db.prepare("SELECT name FROM sqlite_master WHERE type='table'").all();
    return tables.map(t => t.name).filter(name => name.toLowerCase().includes('message') ||
        name.toLowerCase().includes('chat') ||
        name.toLowerCase().includes('msg'));
}
export function getMessagesFromDb(db, options) {
    const messages = [];
    ...
    for (const table of tables) {
        try {
            ...
            console.log(`[IM-DB] Error reading table ${table}:`, e.message);
        }
    }
    return messages;
}
export function getConversationList(db) {
    const conversations = [];
    const tables = db.prepare("SELECT name FROM sqlite_master WHERE type='table'").all();
    for (const table of tables) {
        ...
        if (tableName.includes('buddy') || tableName.includes('roster') || tableName.includes('contact') || tableName.includes('group')) {
            try {
                const results = db.prepare(`SELECT * FROM ${table.name} LIMIT 50`).all();
                for (const row of results) {
                    conversations.push({
                ...
                console.log(`[IM-DB] Error reading table ${table.name}:`, e.message);
            }
        }
    }
    return conversations;
}
export function getMessagesByConversation(db, convId, options) {
    const messages = [];
    const tables = getMessageTables(db);
    for (const table of tables) {
        try {
            const columns = db.pragma(`table_info(${table})`).map(c => c.name.toLowerCase());
            const hasConvId = columns.some((c) => c.includes('gid') || c.includes('group') || c.includes('room') || c.includes('conv'));
            let query = `SELECT * FROM ${table}`;
            const conditions = [];
            const params = [];
            if (hasConvId) {
                const gidCol = columns.find((c) => c.includes('gid') || c.includes('group') || c.includes('room') || c.includes('conv'));
                if (gidCol) {
            ...
            console.log(`[IM-DB] Error reading messages from table ${table}:`, e.message);
        }
    }
    return messages;
}
        ...
        for (const file of dbFiles) {
            ...
            try {
                ...
                try {
                    ...
                    console.log(`[IM-DB] Could not set key pragma: ${e.message}`);
                }
                const tables = db.prepare("SELECT name FROM sqlite_master WHERE type='table'").all();
                db.close();
                return {
                    success: true,
                    message: `Successfully connected to ${file}. Found tables: ${tables.map((t) => t.name).join(', ')}`,
                    keyPreview,
                };
            }
            catch (e) {
                console.log(`[IM-DB] Failed to test ${file}: ${e.message}`);
                continue;
            }
        }
        return {
