...
import { createProjectInitiation, updateProjectInitiation, transitionProjectPhase, addProjectMember, syncProjectStatus, } from "./project-service.js";
import { createRequirement, updateRequirement, deleteRequirement, createDevTask, updateDevTask, assignDevTask, deleteDevTask, } from "./requirement-service.js";
import { upsertDailyPlanWrapper, updateDailyPlanTaskWrapper, deleteDailyPlanTaskWrapper } from "./daily-plan-service.js";
// ---------- 注册函数 ----------
export function registerWriteTools(register) {
    // ===== 新建项目立项 =====
    register({
        definition: {
            name: "create_project",
    ...
    register({
        ...
        execute: async (params) => {
            const result = updateProjectInitiation(params.id, {
                title: params.title,
    ...
    register({
        ...
        requireConfirm: true,
        execute: async (params) => {
            const result = transitionProjectPhase(params.id, params.action, params.reason);
            if (!result.success)
    ...
    register({
        ...
        execute: async (params) => {
            const member = addProjectMember(params.project_id, params.employee_id, params.role || "member");
            if (!member)
                return { error: "添加成员失败，可能已存在" };
            return member;
        },
    });
    // ===== 同步项目状态 =====
    register({
        ...
        execute: async (params) => {
            const result = syncProjectStatus(params.id);
            if (!result)
                return { error: "项目不存在或同步失败" };
            return result;
        },
    });
    // ===== 新建需求分析 =====
    ...
    register({
        ...
        execute: async (params) => {
            const result = updateRequirement(params.id, {
                title: params.title,
                raw_input: params.raw_input,
                status: params.status,
            });
            if (!result)
                return { error: "需求不存在或无变更" };
            return result;
        },
    });
    // ===== 删除需求 =====
    register({
        ...
        requireConfirm: true,
        execute: async (params) => {
            const ok = deleteRequirement(params.id);
            if (!ok)
                return { error: "需求不存在" };
    ...
    register({
        ...
        execute: async (params) => {
            const result = updateDevTask(params.id, {
                status: params.status,
                assignee: params.assignee,
                priority: params.priority,
                estimated_hours: params.estimated_hours,
    ...
    register({
        ...
        execute: async (params) => {
            const result = assignDevTask(params.task_id, params.employee_id);
            if (!result)
                return { error: "任务不存在" };
    ...
    register({
        ...
        requireConfirm: true,
        execute: async (params) => {
            const ok = deleteDevTask(params.id);
            if (!ok)
                return { error: "任务不存在" };
            return { success: true, id: params.id };
        },
    ...
    register({
        ...
        execute: async (params) => {
            ...
            return upsertDailyPlanWrapper(date, goalText, tasks);
        },
    });
    // ===== 更新每日计划任务 =====
    register({
        ...
        execute: async (params) => {
            const ok = updateDailyPlanTaskWrapper(params.task_id, {
                title: params.title,
                status: params.status,
                priority: params.priority,
                assignee: params.assignee,
    ...
    register({
        ...
        requireConfirm: true,
        execute: async (params) => {
            const ok = deleteDailyPlanTaskWrapper(params.task_id);
            if (!ok)
                return { error: "任务不存在" };
            return { success: true, task_id: params.task_id };
        },
...
//# sourceMappingURL=agent-tools-write.js.map