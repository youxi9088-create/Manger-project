...
export class U9ApiClient {
     ...
     * 获取请求头
     */
    getHeaders(uri, method = 'GET', host, origin) {
        const headers = {
    ...
    async searchMessages(params) {
        ...
        if (beginTime)
            queryParams.set('begin_time', beginTime);
        if (endTime)
     ...
     */
    transformSearchResponse(data, convId, myName) {
        const items = data.items || data.data || [];
        // pre=0 表示无更多，pre>0 表示还有更旧消息
        const pre = typeof data.pre === 'number' ? data.pre : (items.length > 0 ? 1 : 0);
     ...
     * 结果按时间升序返回（最旧→最新）
     */
    async getAllMessages(convId, options = {}) {
        const { maxMessages = 100, myName } = options;
        const allMessages = [];
        let beforeMsgId = undefined;
        const limit = 100;
        let pageCount = 0;
...
//# sourceMappingURL=u9-api.js.map