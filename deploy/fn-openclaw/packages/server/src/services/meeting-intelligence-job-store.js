...
export async function saveMeetingIntelligenceJob(job) {
    ...
    if (!response.ok)
        throw new Error('会议分析任务状态保存失败');
}
export async function loadMeetingIntelligenceJob(jobId) {
    const endpoint = storeEndpoint();
    if (!endpoint)
...
//# sourceMappingURL=meeting-intelligence-job-store.js.map