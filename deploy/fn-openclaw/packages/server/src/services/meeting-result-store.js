...
export async function loadRemoteMeetingResult(key) {
...
export async function saveRemoteMeetingResult(result) {
    const endpoint = storeEndpoint();
    if (!endpoint)
        return false;
    ...
    if (!response.ok)
        throw new Error('会议转录持久化失败，请稍后重试。');
    return true;
}
export async function listRemoteMeetingResults(limit) {
...
//# sourceMappingURL=meeting-result-store.js.map