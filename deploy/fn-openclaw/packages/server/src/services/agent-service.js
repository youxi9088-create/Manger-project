 ...
 * 获取员工的 Agent 配置
 */
export function getEmployeeAgentConfig(employeeId) {
    const row = dbInstance.prepare(`SELECT agent_type, agent_prompt, agent_tools, agent_config FROM employees WHERE id = ?`).get(employeeId);
 ...
 * 判断员工是否已启用 Agent 模式
 */
export function isAgentEmployee(employeeId) {
 ...
 * 获取单个角色模板
 */
export function getRoleTemplate(roleId) {
    return AGENT_ROLE_TEMPLATES[roleId] || null;
}
/**
 * 为员工配置/更新 Agent 属性
 */
export function configureEmployeeAsAgent(employeeId, roleId, customPrompt, extraTools) {
    ...
    dbInstance.prepare(`UPDATE employees SET
      agent_type = ?,
      agent_prompt = ?,
      agent_tools = ?,
      agent_config = ?,
      updated_at = datetime('now','localtime')
     WHERE id = ?`).run(roleId, finalPrompt, JSON.stringify(finalTools), JSON.stringify(finalConfig), employeeId);
    // 确认写入成功
 ...
 * 移除员工的 Agent 模式
 */
export function removeAgentFromEmployee(employeeId) {
    dbInstance.prepare(`UPDATE employees SET agent_type = NULL, agent_prompt = NULL, agent_tools = '[]', agent_config = '{}', updated_at = datetime('now','localtime') WHERE id = ?`).run(employeeId);
}
