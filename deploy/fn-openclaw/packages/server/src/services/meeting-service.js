...
import { listRemoteMeetingResults, loadRemoteMeetingResult, saveRemoteMeetingResult, } from './meeting-result-store.js';
// ====== 本地持久化转录结果（用于刷新后保持“已转录”状态）======
// 说明：列表返回的 meetingId 可能不稳定（每次刷新变化），因此这里使用稳定键（title + startTime）来做持久化。
function stableKeyOf(title, startTime) {
    const t = String(title || '').trim();
    const s = String(startTime || '').trim();
    return `${s}__${t}`;
}
function fileSafeSlug(s) {
...
function getResultFilePathByKey(key, dataDir) {
    const slug = fileSafeSlug(key);
    return path.join(dataDir, `result__${slug}.json`);
}
function loadPersistedResultByKey(key, dataDir) {
    try {
...
function listLocalPersistedResults(dataDir, limit) {
...
function savePersistedResultByKey(result, dataDir) {
    try {
...
function generateMacAuth(method, uri, host, authInfo) {
    const diff = globalThis.__uc_diff || 0;
                            ...
                            if (item.name?.includes('ND_UC_AUTH') && item.name?.includes('token')) {
                                try {
                                    ...
                                    if (typeof innerData.diff === 'number') {
                                        globalThis.__uc_diff = innerData.diff;
                                    }
                                }
                                catch (e) {
...
class MeetingAPIService {
    ...
    extendUrl = MEETING_EXTEND_API;
    authKey = Buffer.from(`${USERNAME}:${PASSWORD}`).toString('base64');
    authInfo = null;
    constructor() {
        // 初始化时尝试加载认证信息
    ...
    async fetchMeetingListWithToken() {
        ...
        for (let offset = 0; offset < 1_000; offset += pageSize) {
            ...
            if (!response.ok)
                throw new Error(`会议录制列表请求失败: ${response.status}`);
            const data = await response.json();
    ...
    async downloadMedia(meetingId, format, directUrl, options) {
        ...
        const extFromContentType = (contentType) => {
            ...
            if (ct.includes('ogg'))
                return 'ogg';
            if (ct.includes('m4a') || ct.includes('x-m4a') || ct.includes('aac'))
                return 'm4a';
            return 'mp4';
        };
    ...
    async getMeetingResultLatest(params) {
    ...
    async listMeetingResultVersions(params) {
        const latest = await this.getMeetingResultLatest(params);
        return latest?.versions || [];
    }
    async listMeetingResults(limit = 200) {
    ...
    cleanupOldMeetingMedia(params) {
        ...
        try {
            ...
            for (const f of files) {
                ...
                scanned++;
                // 保留纪要/版本文件
                if (f.startsWith('result__') && f.endsWith('.json'))
                    continue;
                // 仅清理媒体文件
            ...
            return { deleted: 0, scanned: 0, error: String(e?.message || e) };
        }
    }
}
