...
import { getRoleTemplates, getRoleTemplate, configureEmployeeAsAgent, removeAgentFromEmployee, getEmployeeAgentConfig, } from "../services/agent-service.js";
...
router.post("/api/employees/:id/enable-agent", (req, res) => {
    try {
        ...
        if (!role_id)
            return res.status(400).json({ error: "role_id 不能为空，可选: project_delivery" });
        configureEmployeeAsAgent(req.params.id, role_id, custom_prompt, extra_tools);
        const config = getEmployeeAgentConfig(req.params.id);
...
router.post("/api/employees/:id/agent/run", async (req, res) => {
    try {
        ...
        if (!agentConfig)
            return res.status(400).json({ error: "该员工未启用 Agent 模式" });
        const employee = dbInstance.prepare("SELECT name FROM employees WHERE id = ?").get(employeeId);
        if (!employee)
            return res.status(404).json({ error: "员工不存在" });
        const roleTemplate = getRoleTemplate(agentConfig.agent_type || "");
        if (!roleTemplate)
            return res.status(500).json({ error: "未找到角色模板配置" });
        // 构建完整 prompt
...
请开始执行，每完成一个步骤输出进度和结果。如果需要调用工具/API，说明你要做什么。`;
        ...
        try {
            ...
            const sdkSession = await createAgentSession({
                model: agentConfig.agent_config?.model || roleTemplate.model,
            ...
            try {
                res.write(`data: ${JSON.stringify({ type: "error", message: execError.message })}\n\n`);
            }
            catch { /* ignore */ }
        }
        try {
