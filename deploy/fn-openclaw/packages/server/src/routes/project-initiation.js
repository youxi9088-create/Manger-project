...
function generateInitiationId() {
    ...
    const row = dbInstance.prepare(`SELECT id FROM project_initiations WHERE id LIKE ? ORDER BY id DESC LIMIT 1`).get(`${prefix}%`);
    let seq = 1;
    if (row) {
...
router.get("/api/project-initiation/:id", (req, res) => {
    try {
        ...
        if (!row)
            return res.status(404).json({ error: "立项记录不存在" });
...
router.post("/api/project-initiation", (req, res) => {
    try {
        ...
        const { type = 'quick_validation', title, applicant, project_leader, department, project_type, demand_source, demand_date, from_pool = 0, raw_requirement, ai_generated_content, } = req.body;
        const ts = nowStr();
        dbInstance.prepare(`
      ...
      VALUES (?, ?, 'draft', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(id, type, title || null, applicant || null, project_leader || null, department || null, project_type || null, demand_source || null, demand_date || null, from_pool, raw_requirement || null, ai_generated_content || null, ts, ts);
...
router.post("/api/project-initiation/:id/submit", (req, res) => {
    try {
        const existing = dbInstance.prepare("SELECT * FROM project_initiations WHERE id = ?").get(req.params.id); // eslint-disable-next-line @typescript-eslint/no-explicit-any
        if (!existing)
            return res.status(404).json({ error: "立项记录不存在" });
...
router.post("/api/project-initiation/generate", async (req, res) => {
    ...
    if (!raw_requirement)
        return res.status(400).json({ error: "请提供原始需求描述" });
    ...
    try {
        ...
        if (!apiKey) {
            send('error', { message: '未配置 OPENAI_API_KEY' });
            return res.end();
        }
...
router.get("/api/projects/list", (_req, res) => {
    try {
        const rows = dbInstance.prepare(`SELECT id, title, status FROM project_initiations WHERE status IN ('approved', 'submitted', 'pending', 'draft', 'executing') ORDER BY created_at DESC`).all();
        res.json({ success: true, data: rows });
    }
    catch (error) {
        res.status(500).json({ error: error?.message });
...
//# sourceMappingURL=project-initiation.js.map