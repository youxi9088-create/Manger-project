...
router.post("/api/im/local/connect", (req, res) => {
    try {
        ...
        if (!employeeId)
            return res.status(400).json({ error: '工号不能为空' });
        const config = {
            employeeId,
...
router.get("/api/im/local/conversations", (req, res) => {
    try {
        if (!imDbConnection)
            return res.status(400).json({ error: '请先连接本地数据库' });
...
router.post("/api/im/local/import", (req, res) => {
    try {
        if (!imDbConnection || !imDbConfig)
            return res.status(400).json({ error: '请先连接本地数据库' });
        const { sourceId, convId, startTime, endTime, maxMessages = 1000 } = req.body;
        if (!sourceId)
            return res.status(400).json({ error: 'sourceId 不能为空' });
...
router.post("/api/im/local/import-json", (req, res) => {
    try {
        ...
        if (!sourceId || !jsonPath)
            return res.status(400).json({ error: 'sourceId 和 jsonPath 不能为空' });
        const source = db.getImSource(sourceId);
        if (!source)
            return res.status(404).json({ error: '数据源不存在' });
        if (!fs.existsSync(jsonPath))
            return res.status(400).json({ error: 'JSON 文件不存在' });
        const fileContent = fs.readFileSync(jsonPath, 'utf-8');
        const exportData = JSON.parse(fileContent);
        if (!exportData.messages || !Array.isArray(exportData.messages))
            return res.status(400).json({ error: '无效的导出文件格式' });
        const now = new Date().toISOString();
        const dbRecords = exportData.messages.map((msg) => ({
...
//# sourceMappingURL=im-local-db.js.map