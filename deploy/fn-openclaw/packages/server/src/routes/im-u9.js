...
function normalizeU9Timestamp(rawTime, fallback) {
    if (!rawTime)
        return fallback;
        ...
        try {
            ...
            for (const origin of state?.origins || []) {
                for (const entry of origin?.localStorage || []) {
                    if (!String(entry?.name || '').includes('ND_UC_AUTH') || !String(entry?.name || '').includes('token'))
                        continue;
                    ...
                    if (token?.access_token && token?.mac_key) {
                        ...
                        if (!newest || expiresAt > newest.expiresAt)
                            newest = { auth, expiresAt };
                    }
                }
            }
        }
...
function getU9AuthSnapshotPath() {
    const candidates = [
        path.resolve(process.cwd(), 'data', 'auth', 'oa-auth.json'),
        path.resolve(process.cwd(), '..', '..', 'data', 'auth', 'oa-auth.json'),
    ];
...
async function refreshU9Auth(auth) {
...
async function loadU9Conversations(auth) {
    ...
    for (const session of stored)
        conversations.set(session.id, { id: session.id, name: session.name || conversations.get(session.id)?.name || '' });
    ...
    if (live.length > 0 && (saved.length > 0 || stored.length > 0))
        return { sessions, source: 'u9_api_merged' };
    if (live.length > 0)
        return { sessions, source: 'u9_api' };
    if (stored.length > 0)
        return { sessions, source: 'u9_data_store' };
    if (saved.length > 0)
        return { sessions, source: 'local_chat_records' };
    const errors = [groupsResult, friendsResult]
        .filter((result) => result.status === 'rejected')
        .map((result) => result.reason instanceof Error ? result.reason.message : String(result.reason));
    if (errors.length > 0)
        throw new Error(`U9 会话目录读取失败：${errors.join('；')}`);
    return { sessions: [], source: 'u9_api' };
}
async function persistU9Configuration(auth, sessions) {
    ...
    if (auth.refresh_token)
        persistEnvVar('U9_REFRESH_TOKEN', auth.refresh_token);
    if (auth.expires_at)
        persistEnvVar('U9_TOKEN_EXPIRES_AT', auth.expires_at);
    if (auth.user_id)
        persistEnvVar('U9_USER_ID', String(auth.user_id));
...
function readLegacyU9Conversations() {
    const envContent = readEnvFileContent();
 ...
 */
export async function ensureU9Configuration() {
    const envContent = readEnvFileContent();
    const readValue = (key) => process.env[key]?.trim()
        || envContent.match(new RegExp(`^${key}=(.*)$`, 'm'))?.[1]?.trim()
        || '';
    ...
    const envAuth = authId && authKey && appId ? {
        access_token: authId,
        mac_key: authKey,
...
router.post("/api/im/chat-records/import-u9", async (req, res) => {
    try {
        ...
        if (!sourceId || !convId)
            return res.status(400).json({ error: "sourceId 和 convId 不能为空" });
...
router.post("/api/im/chat-records/search-u9", async (req, res) => {
    try {
        ...
        if (!convId)
            return res.status(400).json({ error: "convId 不能为空" });
        await ensureU9Configuration();
        const u9Client = createU9Client();
...
router.post("/api/im/refresh-conversations", async (req, res) => {
    try {
        ...
        if (!authId || !authKey || !appId)
            return res.status(400).json({ error: '99U API 未配置' });
        ...
        const [groups, friends] = await Promise.all([
            u9Client.getGroups(userId).catch(() => []),
            u9Client.getAllFriends().catch(() => []),
        ]);
        const existingIds = new Set(configured.sessions.map((session) => session.id));
        const existingNames = Object.fromEntries(configured.sessions.map((session) => [session.id, session.name]));
        const previousCount = existingIds.size;
        const newConversations = [];
        for (const g of groups) {
            if (!existingIds.has(g.id)) {
                existingIds.add(g.id);
                newConversations.push({ id: g.id, name: g.name, type: 'group' });
            }
            if (g.name)
                existingNames[g.id] = g.name;
        }
        for (const f of friends) {
            if (!existingIds.has(f.id)) {
                existingIds.add(f.id);
                newConversations.push({ id: f.id, name: f.name, type: 'friend' });
            }
            if (f.name)
                existingNames[f.id] = f.name;
        }
...
// ============= 自动登录（SSE）=============
let autoSetupBrowser = null;
let autoSetupSmsResolve = null;
    ...
    try {
        ...
        if (!url.includes('login') && url.includes('ndim.101.com')) {
            ...
            if (!loggedInByQr) {
                ...
                if (pcSwitcher) {
                    try {
                        await pcSwitcher.click({ force: true, timeout: 5000 });
                    }
                    catch {
                        await page.evaluate(() => { document.querySelector('.login-qrcode-switcher__pc')?.click(); });
                    }
                ...
                let accountFrame = null, passwordFrame = null;
                // 在主页面查找
                ...
                if (smsInput) {
                    ...
                    if (getCodeBtn)
                        await getCodeBtn.click({ force: true });
                    const smsCode = await new Promise((resolve) => {
                        autoSetupSmsResolve = resolve;
                        setTimeout(() => { if (autoSetupSmsResolve === resolve) {
                            autoSetupSmsResolve = null;
                            resolve('');
                        } }, 5 * 60 * 1000);
                    });
                    if (!smsCode) {
                        send('error', { message: '验证码超时' });
                        await browser.close();
                        autoSetupBrowser = null;
                        res.end();
                        return;
        ...
        const persistedAuth = { ...authInfo, refresh_token: refreshedSnapshot.refresh_token };
        ...
        send('done', {
            ...
            conversationIds: sessions.map((s) => s.id),
        });
        res.end();
    }
    catch (error) {
