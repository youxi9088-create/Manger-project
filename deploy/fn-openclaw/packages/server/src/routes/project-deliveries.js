...
router.get("/api/projects/:id/deliveries", (req, res) => {
    try {
        const { id } = req.params;
        const rows = dbInstance.prepare(`SELECT * FROM versions WHERE project_id = ? ORDER BY created_at DESC`).all(id);
...
router.post("/api/projects/:id/deliveries", (req, res) => {
    try {
        ...
        dbInstance.prepare(`INSERT INTO versions (id, name, status, project_id, expected_release_date, description, task_ids, created_at, updated_at)
       VALUES (?, ?, 'pending_confirm', ?, ?, ?, ?, ?, ?)`).run(vid, name, id, expected_release_date || null, description || null, task_ids ? JSON.stringify(task_ids) : "[]", ts, ts);
...
//# sourceMappingURL=project-deliveries.js.map