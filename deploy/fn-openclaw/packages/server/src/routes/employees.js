...
router.get("/api/employees", (_req, res) => {
    try {
        ...
        const params = []; // eslint-disable-line @typescript-eslint/no-explicit-any
...
router.post("/api/employees", (req, res) => {
    try {
        ...
        dbInstance.prepare(`INSERT INTO employees (id, name, rank, skills, access_method, status, power_level, avatar_url, notes, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, name, rank || null, skills || null, access_method || "local", status || "available", power_level ?? 50, avatar_url || null, notes || null, now, now);
...
router.post("/api/employees/:id/generate-avatar", async (req, res) => {
    try {
        const emp = dbInstance.prepare("SELECT id, name, rank, skills FROM employees WHERE id = ?").get(req.params.id);
        if (!emp)
            return res.status(404).json({ error: "员工不存在" });
