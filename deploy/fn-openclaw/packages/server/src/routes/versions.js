...
function generateRequirementId() {
    ...
    const row = dbInstance.prepare(`SELECT id FROM requirement_analyses WHERE id LIKE ? ORDER BY id DESC LIMIT 1`).get(`${pfx}%`);
    let seq = 1;
    if (row) {
...
router.get("/api/versions", (_req, res) => {
    try {
        const rows = dbInstance.prepare(`SELECT * FROM versions ORDER BY created_at DESC`).all();
        // 为每个版本计算最新预警（不阻塞：静默失败）
        for (const v of rows) {
            try {
                computeVersionRisk(v.id);
            }
            catch { /* ignore */ }
        }
        // 重新查一次拿到更新后的预警数据
...
router.get("/api/versions/:id", (req, res) => {
    try {
        try {
            computeVersionRisk(req.params.id);
        }
        catch { /* ignore */ }
        ...
        // 关联的需求
        const requirements = dbInstance.prepare("SELECT * FROM requirement_analyses WHERE version_id = ? ORDER BY created_at DESC").all(req.params.id);
...
router.post("/api/versions", (req, res) => {
    ...
    `).run(id, name, risk_level || "medium", participants || "[]", expected_release_date || null, description || null, task_ids || "[]", goalsJson, project_id || null);
        try {
            computeVersionRisk(id);
        }
        catch { /* ignore */ }
...
router.post("/api/versions/:id/goals", (req, res) => {
    try {
        ...
        const ver = dbInstance.prepare("SELECT goals FROM versions WHERE id = ?").get(req.params.id);
        if (!ver)
            return res.status(404).json({ error: "版本不存在" });
...
router.patch("/api/versions/:id/goals/:goalId", (req, res) => {
    try {
        ...
        if (!ver)
            return res.status(404).json({ error: "版本不存在" });
        const goals = JSON.parse(ver.goals || '[]');
        const idx = goals.findIndex((g) => g.id === req.params.goalId);
        if (idx < 0)
...
router.delete("/api/versions/:id/goals/:goalId", (req, res) => {
    try {
        ...
        const newGoals = goals.filter((g) => g.id !== req.params.goalId);
        dbInstance.prepare("UPDATE versions SET goals = ?, updated_at = datetime('now','localtime') WHERE id = ?")
...
router.post("/api/versions/:id/goals/:goalId/convert", (req, res) => {
    try {
        ...
        if (!ver)
            return res.status(404).json({ error: "版本不存在" });
        const goals = JSON.parse(ver.goals || '[]');
        const goalIdx = goals.findIndex((g) => g.id === req.params.goalId);
        if (goalIdx < 0)
            return res.status(404).json({ error: "目标不存在" });
...
router.get("/api/versions/:id/requirements", (req, res) => {
    try {
        const rows = dbInstance.prepare("SELECT * FROM requirement_analyses WHERE version_id = ? ORDER BY created_at DESC").all(req.params.id);
