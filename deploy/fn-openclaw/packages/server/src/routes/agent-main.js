...
import { prepareHermesSession, runHermesChat, } from "../services/hermes-service.js";
initAgentTools();
const router = Router();
// ============= POST /api/agent/chat（Hermes 对话入口）============
router.post("/api/agent/chat", async (req, res) => {
    const { sessionId: _sessionId, message } = req.body;
    if (!message?.trim()) {
...
router.get("/api/agent/tasks", (_req, res) => {
    try {
        res.json({ success: true, data: dbInstance.prepare("SELECT id, session_id, description, status, agent_type, iterations, created_at, completed_at FROM agent_tasks ORDER BY created_at DESC LIMIT 50").all() });
    }
    catch (error) {
        res.status(500).json({ error: error instanceof Error ? error.message : "查询失败" });
    }
...
router.get("/api/agent/runtime-status", (_req, res) => {
    try {
        const mainStats = dbInstance.prepare("SELECT COUNT(*) as total FROM sessions").get();
        const mainLast = dbInstance.prepare("SELECT updated_at FROM sessions ORDER BY updated_at DESC LIMIT 1").get();
        const execRunning = dbInstance.prepare("SELECT id, description, created_at FROM agent_tasks WHERE status='running' ORDER BY created_at DESC LIMIT 1").get();
        const execTotalRow = dbInstance.prepare("SELECT COUNT(*) as total FROM agent_tasks WHERE agent_type='exec'").get();
        const execTotal = execTotalRow?.total || 0;
        const execOkRow = dbInstance.prepare("SELECT COUNT(*) as ok FROM agent_tasks WHERE agent_type='exec' AND status='completed'").get();
        const execOk = execOkRow?.ok || 0;
        const execLastDone = dbInstance.prepare("SELECT completed_at FROM agent_tasks WHERE status='completed' ORDER BY completed_at DESC LIMIT 1").get();
        res.json({
            success: true,
            data: {
...
//# sourceMappingURL=agent-main.js.map